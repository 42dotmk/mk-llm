...
def fix_translation(original_text=ENGLISH_TEXT, translated_text=INITIAL_TRANSLATED_TEXT):
    ...
    while not (illogical_sentences_fixed and gramatical_errors_fixed) or iteration < max_iterations:
        ...
        for sentence in sentences_for_replacing:
            if sentence['original_translation'] in translated_text:
                translated_text = translated_text.replace(sentence['original_translation'], sentence['fixed_translation'])
        ...
        if len(remaining_sentences)>0:
            fixed_translation=replace_fixed_sentences_in_text(original_text=original_text,translation_text=translated_text, fixed_sentences=sentences_for_replacing)
